...
- 导出错题集为错题本。
- 自定义每一面题目的数量。
- 内置 A4、B5 和平板横屏 4:3 / 16:10 等尺寸，也可添加、更新或删除自定义纸张预设。
- 中间文件始终在应用专用工作区中处理，输出目录只接收完的成品 PDF。
...
3. 选择纸张和每页题目数（1–12）。平板可选「平板横屏 4:3」或「平板横屏 16:10」；点击「管理预设…」可保存自定义名称和宽高。右侧示意会同步更新。
...
生成的成品尺寸与所选纸张一致。开启封面后，它会成为 PDF 的第一页；标题最长 64 个字符，描述最长 240 个字符，封面图片支持 JPG、PNG、WebP、BMP、TIFF。图片会等比完整显示并居中放在标题上方。来源文件与成品不能同名同路径；加密 PDF 需要先解锁。右侧显示的是排版示意，不是题目内容预览。
